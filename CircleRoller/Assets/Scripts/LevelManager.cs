...
public class LevelManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        _levelConfig[0] = FillLevelDescriptor(0, 10.0f, 0.30f, 0.0020f, 2.0f, 2.0f/1, 5,  0.85f, 0.85f);
    ...
    private LevelDescriptor FillLevelDescriptor(int idx, float rotationSpeed, float movementThreshold, float increazingSizeSpeed, float laserDestroyTime, float timeBetweenLaserSpawns, int scoreToNextLevel, float minimumCircleSize, float maximumCircleSize)
    {
        ...
        //level_descriptor.circleSize = circleSize;
    ...
    /*public void SetNextLevel(int idx)
    {
        _nextLevel = idx;
        if (idx < 0)
        {
        ...
        else if (idx > (_levelConfig.Length - 1))
        {
            _nextLevel = _levelConfig.Length - 1;
        }
        else
        {
            _nextLevel = idx;
